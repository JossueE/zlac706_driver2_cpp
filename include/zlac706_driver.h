...
public:
    ...
    std::vector<unsigned char> str_msg = std::vector<unsigned char>{startCommad, 0, 1, 1};  ///< Motor start, Write motor enable
    std::vector<unsigned char> stp_msg = std::vector<unsigned char>{startCommad, 0, 0, 0};  ///< Motor stop, Write motor disable
    std::vector<unsigned char> spm_msg = std::vector<unsigned char>{speedMCommand, 0, 0xC4, 0xC6};  ///< Speed mode selection, Control mode given command source selection
    std::vector<unsigned char> alm_msg = std::vector<unsigned char>{alarmCommand, 0, 0, alarmCommand};  ///< Clear fault
    std::vector<unsigned char> cfg_msg = std::vector<unsigned char>{configCommand, 0, 0, 0}; ///< Speed mode
    std::vector<unsigned char> sSpeed_msg = std::vector<unsigned char>{setSpeedCommand, 0, 0, 0}; ///< Speed debug mode
    std::vector<unsigned char> kpG_msg = std::vector<unsigned char>{setKpCommand, 0, 0, 0}; ///<  Speed proportional gain
    std::vector<unsigned char> kiG_msg = std::vector<unsigned char>{setKiCommand, 0, 0, 0}; ///<  Speed integral gain
    std::vector<unsigned char> kdG_msg = std::vector<unsigned char>{setKdCommand, 0, 0, 0}; ///<  Speed differential gain

    std::vector<unsigned char> gVolt_msg = std::vector<unsigned char>{getVoltCommand, 0, 0, getVoltCommand}; ///< Bus voltage
    std::vector<unsigned char> gAmps_msg = std::vector<unsigned char>{getAmpsCommnad, 0, 0, getAmpsCommnad}; ///< Output current
    std::vector<unsigned char> gSpeed_msg = std::vector<unsigned char>{getSpeedCommand, 0, 0, getSpeedCommand}; ///< Motor speed
    std::vector<unsigned char> gTicks_msg = std::vector<unsigned char>{getTicksCommand, 0x00, 0x00, getTicksCommand}; ///< Position feedback
    std::vector<unsigned char> gParams_msg = std::vector<unsigned char>{readParamsCommand, 0x00, 0x14, 0x5D};

    std::vector<unsigned char> alv_msg = std::vector<unsigned char>{aliveCommand, 0, aliveCommand};

    std::vector<unsigned char> alm_status = std::vector<unsigned char>{alarmstatus, 0x00, 0x00, alarmstatus};
...
private:
    ...
    std::vector<unsigned char> commandResponse = std::vector<unsigned char>(5, (unsigned char)0); ///< Message response 
    std::vector<unsigned char> ticks_v = std::vector<unsigned char>(4, (unsigned char)0); ///< Message response for ticks

    std::vector<unsigned char> messageE8 = std::vector<unsigned char>(2, (unsigned char)0); ///< Message response for E8
    std::vector<unsigned char> messageE9 = std::vector<unsigned char>(2, (unsigned char)0); ///< Message response for E9
