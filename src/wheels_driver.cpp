...
public:
  ZlacNode(const std::string &port_left, const std::string &port_right) : Node(node_name), port_left_(port_left), port_right_(port_right) {
    ...
    this->declare_parameter<std::vector<int64_t>>("left_velocity_gains",std::vector<int64_t>{1500, 20, 100});
    this->declare_parameter<std::vector<int64_t>>("right_velocity_gains",std::vector<int64_t>{1500, 20, 100});
    ...
    //serial_left_.setCallback(received());
    ...
    // if (can_id_ != motor_left_.can_id) {
    //     RCLCPP_WARN(rclcpp::get_logger(node_name), "CAN Device not recognized");
    //     RCLCPP_INFO(rclcpp::get_logger(node_name), "Can ID is %d", motor_left_.can_id);
    //     rclcpp::shutdown();
    // }
  ...
  ~ZlacNode() override {
    try {
      //motor_left_.configAcc(2000, 2000);
      //serial_left_.write(motor_left_.cfg_msg);

      //motor_right_.configAcc(2000, 2000);
      //serial_right_.write(motor_right_.cfg_msg);
